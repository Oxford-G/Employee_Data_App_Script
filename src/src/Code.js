...
function editRecord() {
  ...
  for(i=0; i < values.length; i++) {
    ...
    if(rowValue[0]==str) {
      ...
      dataSheet.getRange(iRow,2).setValue(shUserForm.getRange("C9").getValues()); // Employee Name

      dataSheet.getRange(iRow,3).setValue(shUserForm.getRange("C11").getValues()); // Gender

      dataSheet.getRange(iRow,4).setValue(shUserForm.getRange("C13").getValues()); // Email Id

      dataSheet.getRange(iRow,5).setValue(shUserForm.getRange("C15").getValues()); // Department

      dataSheet.getRange(iRow,6).setValue(shUserForm.getRange("C17").getValues()); // Address

      //Submitted On

      dataSheet.getRange(iRow,7).setValue(new Date()).setNumberFormat('yyyy-mm-dd h:mm');

      //Submitted by

      dataSheet.getRange(iRow,8).setValue(Session.getActiveUser().getEmail());
